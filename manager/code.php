...
if (isset($_POST['saveUser'])) {
    ...
    if ($_FILES['image']['size'] > 0) {
        ...
        $finalImage = '/assets/uploads/users/' . $filename;
    } else {
        $finalImage = NULL;
    }

    if ($name != '' && $email != '' && $phone != '' && $password != '' && $center != '') {
...
if (isset($_POST['updateUser'])) {
    ...
    if ($_FILES['image']['size'] > 0) {
        ...
        $imgFileTypes = strtolower(pathinfo($image, PATHINFO_EXTENSION));
        if ($imgFileTypes != 'jpg' && $imgFileTypes != 'jpeg' && $imgFileTypes != 'png') {
            redirect('users-edit.php', 'Sorry , only JPG, JPEG and PNG images are supported');
        }
